...
def proof_of_work(prev_state_hash, nonce, payload):
    # obviously we need to do big math here
    return hashlib.sha256(prev_state_hash + nonce + payload).hexdigest()
...
class BlockVerifier(BlockVerifierInterface):
    ...
    def verify_block(self, block_wrapper):
        return block_wrapper.header.consensus == b"Truss"
...
class ForkResolver(ForkResolverInterface):
    ...
    def compare_forks(self, cur_fork_head, new_fork_head):
        ...
        if new_fork_head.consensus != b"Truss":
            raise \
                TypeError(
        ...
        # against.
        if cur_fork_head.consensus != b"Truss":
